    ...
    public partial class BankAccountDetailsManagement : UserControl
    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (comboBox1.SelectedItem == "ID Number")
            {
                try
                {
                    SqlCommand cmd = new SqlCommand("SELECT * FROM BANK_ACCOUNT_DETAILS where  ID_Number LIKE '%" + txt_Search.Text + "%'", conn);
            ...
            else if (comboBox1.SelectedItem == "First Name")
            {
                try
                {
                    SqlCommand cmd = new SqlCommand("SELECT * FROM BANK_ACCOUNT_DETAILS where First_Name LIKE '%" + txt_Search.Text + "%'", conn);
                    SqlDataAdapter da = new SqlDataAdapter(cmd);
                    DataTable dt = new DataTable();
                    da.Fill(dt);
                    bunifuCustomDataGrid1.DataSource = dt;
